...
class LightSource:
    ...
    laser_linewidth_nm: float = 0.01   # 仅 laser 使用
...
def generate_spectrum(light: LightSource):
    ...
    if light.type == "gaussian":
        ...
        return laser_spectrum_k(
            light.central_wavelength_nm,
            light.laser_linewidth_nm
        )
...
#              单色激光
# =========================================================
def laser_spectrum_k(lambda0_nm, linewidth_nm):
    lambda0_m = lambda0_nm * 1e-9
    k0 = 2 * np.pi / lambda0_m

    # 用极窄高斯近似 δ 函数
    delta_k = 2 * np.pi * linewidth_nm * 1e-9 / (lambda0_m ** 2)

    k = np.array([k0 - delta_k, k0, k0 + delta_k])
    S = np.array([0.1, 0.8, 0.1])
