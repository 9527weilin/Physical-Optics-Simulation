...
def create_optical_plot(params, z_mm, mag, label_size):
    ...
    try:
        ...
        width_inch = label_size.width() / 100  # 转换为英寸
        height_inch = label_size.height() / 100  # 转换为英寸
        ...
        # 计算观察平面的距离
        if params.mode == 0:
            observation_distance = z_mm  # 无透镜，直接是物距
        else:
        ...
        # 缩放以适应QLabel - 这里使用Stretch模式，使图像填满整个空间
        scaled_pixmap = pixmap.scaled(
            label_size,
            QtCore.Qt.IgnoreAspectRatio,  # 忽略宽高比，填满整个空间
        ...
        # 返回一个错误信息的QPixmap
        error_pixmap = QtGui.QPixmap(label_size)
        error_pixmap.fill(QtGui.QColor(240, 240, 240))
        painter = QtGui.QPainter(error_pixmap)
        painter.setPen(QtGui.QColor(255, 0, 0))
        painter.setFont(QtGui.QFont("Arial", 10))
        painter.drawText(error_pixmap.rect(), QtCore.Qt.AlignCenter, 
                        f"光路图绘制失败:\n{str(e)[:50]}")
        painter.end()
        return error_pixmap