...
基于PyQt5的GUI界面，集成菲涅尔衍射和透镜成像模拟功能
优化版本：原图和成像图有工具栏，光路图无工具栏，无colorbar
PlotAll: 绘制光路图和成像图
ZvalueSim: 模拟屏幕在像距附近移动
"""
...
class ImgApp(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.simulation_direction = 1  # 1: 增加, -1: 减少
        self.simulation_step = 0
        self.simulation_total_steps = 50  # 总步数
        self.z_sim_min = 0
        self.z_sim_max = 0
    ...
    def set_default_values(self):
        ...
        # 像素个数
        self.ui.PixelN.setText(str(default_params.Nx))
    ...
    def set_lens_mode(self, mode):
        ...
        if mode == 0:
            self.ui.Lensf1.setEnabled(False)
            self.ui.Lensf2.setEnabled(False)
            self.ui.f1Tof2.setEnabled(False)
        elif mode == 1:
            self.ui.Lensf1.setEnabled(True)
            self.ui.Lensf2.setEnabled(False)
            self.ui.f1Tof2.setEnabled(False)
        else:  # mode == 2
            self.ui.Lensf1.setEnabled(True)
            self.ui.Lensf2.setEnabled(True)
            self.ui.f1Tof2.setEnabled(True)
    ...
    def connect_signals_slots(self):
        ...
        self.ui.PixelN.textChanged.connect(self.update_params)
    ...
    def plot_all(self):
        ...
        try:
            # 显示等待消息
            self.ui.LightroadShow.setText("正在计算中...")
            ...
            # 计算图像质量指标
            metrics = calculate_image_metrics(self.U0, I)
    ...
    def start_z_simulation(self):
        ...
        try:
            ...
            # 开始定时器模拟
            self.simulation_timer.start(50)  # 每50ms更新一次
