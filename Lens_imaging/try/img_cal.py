...
# 圆形孔径函数
# -----------------------------
def aperture_mask(p: OpticalParams, radius_mm):
    radius = radius_mm
    Lx = p.Nx * p.dx
    Ly = p.Ny * p.dy
...
# 透镜相位叠加
# -----------------------------
def lens_phase(U0, p: OpticalParams, f: float, radius_mm=25e-3):
    ...
    mask = aperture_mask(p, radius_mm)
    H_lens *= mask
...
#     h = np.exp(1j * k * (X**2 + Y**2) / (2 * z))  # 菲涅尔衍射的传播核
...
def fresnel_propagation_fft(U0, p: OpticalParams, z, pad_factor=2):
    ...
    # 在中心放置原图，周围补零
    U0_pad = np.zeros((Ny_pad, Nx_pad), dtype=complex)
    start_x = (Nx_pad - Nx) // 2
    start_y = (Ny_pad - Ny) // 2
    U0_pad[start_y:start_y+Ny, start_x:start_x+Nx] = U0
    ...
    # 裁剪回原始尺寸
    result = Uz_pad[start_y:start_y+Ny, start_x:start_x+Nx]
    return result
...
#     # 创建频率坐标
#     fx = np.fft.fftfreq(Nx, p.dx)
#     fy = np.fft.fftfreq(Ny, p.dy)
#     FX, FY = np.meshgrid(fx, fy)
...
#     #逆变换回空域
#     Uz = np.fft.ifftshift(np.fft.ifft2(Uz_fft))
...
def geometric_focus_and_mag(p: OpticalParams):
    if p.mode == 1:
        ...
        # 第二个透镜的成像
        s2 = p.f1Tof2 - s1_prime  # 从透镜1的像到透镜2的距离
        f2 = p.f2
...
def simRun(params: OpticalParams, U0):
    # ------------------ 透镜处理 ------------------
    if params.mode == 0:
        U_lens = U0
    elif params.mode == 1:
        U0_img = fresnel_propagation_fft(U0, params, params.imageTolens)
        U_lens = lens_phase(U0_img, params, params.f1, radius_mm=params.radius_mm1)
    elif params.mode == 2:
        U1 = fresnel_propagation_fft(U0, params, params.imageTolens)
        U1 = lens_phase(U1, params, params.f1, radius_mm=params.radius_mm1)
        U2 = fresnel_propagation_fft(U1, params, params.f1Tof2)
        U_lens = lens_phase(U2, params, params.f2, radius_mm=params.radius_mm2)
    ...
    # 如果I有值大于1，设为1
    I[I > 1] = 1.0
    return Uz, U_lens, I, z_init, mag


# ----------------------------- 主函数 -----------------------------
if __name__ == "__main__":
    img_path = r"photo\cat.png"
    U0, Nx, Ny = load_image(img_path, dx=6.5e-6, dy=6.5e-6)
    # print(f"图像尺寸: Nx={Nx}, Ny={Ny}")
    # print(f"photo U0 shape: {U0.shape}")
    ...
    Uz, U_lens, I, z_init, mag = simRun(params, U0)
    # ------------------ 显示 ------------------
